                        ...
                        <div class="card-body">
                            <div class="table-responsive">
                                ...
                                <tbody>
                                    <?php
                                    $no = 1;
                                    $query = "SELECT fuel_slips.*,
                                                     drivers.firstName AS driverFirstName,
                                                     drivers.middleName AS driverMiddleName,
                                                     drivers.lastName AS driverLastName,
                                                     staffs.firstName AS staffFirstName,
                                                     staffs.middleName AS staffMiddleName,
                                                     staffs.lastName AS staffLastName,
                                                     garbage_trucks.plateNumber
                                              FROM fuel_slips
                                              LEFT JOIN drivers ON fuel_slips.driver_id = drivers.id
                                              LEFT JOIN staffs ON fuel_slips.staff_id = staffs.id
                                    ...
                                    if (mysqli_num_rows($query_run) > 0) {
                                        foreach ($query_run as $row) {
                                            ...
                                            $driverFullName = '';
                                            if ($row['driverFirstName']) {
                                                $driverFullName = ucwords($row['driverFirstName']);
                                                if ($row['driverMiddleName']) {
                                                    $driverFullName .= ' ' . strtoupper(substr($row['driverMiddleName'], 0, 1)) . '.';
                                                }
                                                $driverFullName .= ' ' . ucwords($row['driverLastName']);
                                            }
                                            
                                            $staffFullName = '';
                                            if ($row['staffFirstName']) {
                                                $staffFullName = ucwords($row['staffFirstName']);
                                                if ($row['staffMiddleName']) {
                                                    $staffFullName .= ' ' . strtoupper(substr($row['staffMiddleName'], 0, 1)) . '.';
                                                }
                                                $staffFullName .= ' ' . ucwords($row['staffLastName']);
                                            }
                                            ?>
                                            <tr style="text-align:center">
                                                <td><?php echo $no; ?></td>
